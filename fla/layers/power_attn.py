...
class PowerAttention(nn.Module):
    r"""
    The layer implementaion for Power Attention.  # noqa

    Args:
        mode (str, Optional):
            Which Power Attention kernel to use.
    ...
    def __init__(
        ...
        assert self.key_dim == self.value_dim, "key dim and value dim must be the same, but got key_dim: {self.key_dim} and value_dim: {self.value_dim}"
    ...
    def forward(
        ...
        **kwargs
        ...
        if attention_mask is not None:
            raise NotImplementedError("Not supported: custom attention mask for Power Attention.")
        if self.num_kv_groups > 1:
            raise NotImplementedError("Not supported: MQA for Power Attention.")
        if not (past_key_values is None or len(past_key_values) == 0):
            raise NotImplementedError("Not supported: past key values for Power Attention.")
        ...
        gk = self.gk_proj(hidden_states).to(dtype=torch.float32)
    ...
    def state_size(self, **kwargs) -> int:
        raise NotImplementedError("Not yet implemented: state size for Power Attention.")
